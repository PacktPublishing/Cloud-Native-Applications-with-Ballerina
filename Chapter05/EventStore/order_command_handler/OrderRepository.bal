...
public class OrderRepository {
    ...
    public function createEntity(string entityId, string entityType, Event event) returns error? {
        transaction {
            ...
            VALUES(${event.eventId}, ${event.eventType}, ${event.entityType}, ${event.entityId}, ${event.eventData.toString()}, ${event.timestamp.toString()})`; 
    ...
    public function readEvents(string entityId) returns @untainted Event[]|error {
        ...
        Event[] eventList = [];
        error? e = entityItemStream.forEach(function(Event event) {
            eventList.push(event);
        });
        return eventList;       
    }
    public function readEventsTo(string entityId, time:Utc timeTo) returns @untainted Event[]|error {
        stream<record{}, error> resultStream = self.jdbcClient->query(`SELECT * FROM Events WHERE 
        EntityId=${entityId} AND Timestamp < ${timeTo.toString()}`, Event);
        stream<Event, sql:Error> entityItemStream = <stream<Event, sql:Error>>resultStream;
        Event[] eventList = [];
        error? e = entityItemStream.forEach(function(Event event) {
            eventList.push(event);
    ...
    public function removeEventTo(string entityId, time:Utc timeTo) returns error? {
        sql:ParameterizedQuery createOrder = `DELETE FROM Events WHERE EntityId = ${entityId} AND Timestamp < ${timeTo.toString()}`; 
        sql:ExecutionResult result = check self.jdbcClient->execute(createOrder);
        return; 
    }
    ...
    public function getAllEntities() returns @untainted Entity[]|error {
        ...
        Entity[] entities = [];
        error? e = orderItemStream.forEach(function(Entity entity) {
    ...
    public function addSnapshot(Snapshot snapshot) returns error? {
        ...
        ${snapshot.message}, ${snapshot.timestamp.toString()})`; 
        sql:ExecutionResult result = check self.jdbcClient->execute(createOrder);
        return;  
    }
