...
OrderTable orders = table [{
    orderId: "43234234",
    customerEmail: "teairraf9@kweekendci.com",
    inventoryEmail: "teairraf9@kweekendci.com",
...
public function sendDeliveryRequestMail(awslambda:Context ctx, json item) returns json|error {
    ...
    https://ljcm9po77j.execute-api.us-west-1.amazonaws.com/staging/confirmdeliveryrequest?orderId=${orderId}&taskToken=${encodedTaskToken}"`;
...
function sendEmail(string to, string subject, string body) returns error?{
        email:SmtpClient smtpClient = check new ("in-v3.mailjet.com",
        "0f06b04d129da845e6f3b0636400178b" , "43ce77969dba92df31e95dd71b9a246e");
    email:Message email = {
        to: [to],
        subject: subject,
        body: body,
