...
service /OrderAPI  on new http:Listener(9090) { 
    resource function post addNewOrder(http:Caller caller, http:Request req, @http:Payload json orderDetails) returns error? { 
        ...
        if customerId is json && shippingAddress is json {
            error|string orderId = createOrder(shippingAddress.toString(), customerId.toString());
            if orderId is string {
                ...
                log:printError("Error creating database link", 'error=orderId);
                check responseError500(caller);
            }
        } else {
            log:printError("Error reading input values ");
            check responseError500(caller);
        }
    }
    resource function post addCustomer(http:Caller caller, http:Request req, @http:Payload json customerDetails) returns error? { 
    ...
    resource function post addProduct(http:Caller caller, http:Request req, @http:Payload json productDetails) returns error? { 
    ...
    resource function post addSupplier(http:Caller caller, http:Request req, @http:Payload json supplierDetails) returns error? { 
    ...
    resource function post addInventory(http:Caller caller, http:Request req, @http:Payload json inventoryDetails) returns error? { 
